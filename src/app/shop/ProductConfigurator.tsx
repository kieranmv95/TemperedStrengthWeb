...
export default function ProductConfigurator({ product }: ProductConfiguratorProps) {
  ...
  const selectionSummary = [selectedColor, selectedSize].filter(Boolean).join(" · ");
  ...
  const mobileBuyBar = (
    <div className="fixed inset-x-0 bottom-0 z-50 border-t border-neutral-800 bg-[#0a0a0a] px-4 py-3 lg:hidden">
      <div className="mx-auto flex max-w-6xl items-center gap-3">
        <div className="min-w-0 flex-1">
          {selectionSummary ? (
            <p className="truncate text-xs text-neutral-500">{selectionSummary}</p>
          ) : null}
          <p className="text-lg font-semibold leading-tight text-[#c9b072]">
            {priceLabel}
          </p>
        </div>
        <BuyButton
          ...
          fullWidth
          className="w-[min(100%,11rem)] shrink-0"
        />
      </div>
    </div>
  );
  ...
  return (
    <>
      <article className="overflow-hidden rounded-2xl border border-neutral-800 bg-neutral-900/50 p-4 backdrop-blur-sm sm:p-6 md:p-8 lg:pb-8">
      ...
      {mounted ? createPortal(mobileBuyBar, document.body) : null}
    </>
  );
}
